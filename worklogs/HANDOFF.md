...
**Work on hold:**
1. implementation-plans/loop-improvement-workplan.md
...
2. implementation-plans/mutation-survivors-workplan.md
   Active mutation survivor clearing plan.
   Cross-ref: ./BACKLOG.md I-23.
...
**As of:** 2026-07-19 — PR #53 observability work session 2. Key accomplishments this session:

1. **Skill rewrite:** `knowledge/skills/tests-writing/SKILL.md` — integrated rewrite incorporating 10 gap fixes (two-sided contracts, path inventory, dual-write consistency, kill-check targeting producer, matrix coverage gate, vacuous-pass detection, C0 false-confidence trap, etc.). 6 new invariants (I-TW-14 through I-TW-19).

2. **FINDING-V2 fixed:** Circuit breaker path in coder_loop.py (~L920) now emits `context_warn(action="stage3")` — was missing the OutputEvent emit while the other stage3 path (L974) had it. Test: `test_circuit_breaker_emits_context_warn`.

3. **FIX-3 wired:** spawn_subagent.py now emits `subagent_start` and `subagent_end` via `state.output_bus` at all 3 code paths (start, done/fail, error). Was only logging to EventLog.

4. **Dual-write gap fixed:** AFTER_LLM_CALL hook_deny path in coder_loop.py (~L1232) now emits `hook_deny` OutputEvent — was only logging to EventLog (the BEFORE_LLM_CALL path already had the emit).

5. **C1 producer tests added:** 6 new tests in `tests/test_event_type_c1_producers.py` covering all previously-untested EventTypes: session_start, turn_start, llm_response, tool_call, hook_deny, subagent_start/end.

6. **Contract check wired into CI:** `scripts/check_producer_consumer_contract.py` added to `just check` (both justfile and Makefile). C1 test coverage upgraded from warning (⚠️) to hard gate (❌ exit 1).

7. **Contract check now fully passes:** 14 EventType literals, 14 handlers, 13/13 non-dormant have both producer and consumer, 13/13 have C1 tests. Exit 0.

**Test count:** 47 tests pass (38 original + 2 os-shadowing + 1 FINDING-V2 + 6 C1 producers).
...
top of `README.md`). PR notes/bodies → `./pr-notes/` (+ index);
...
Policy change: `knowledge/README.md` + `doc-maintenance` skill now allows deliberate
...
`implementation-plans/mutation-survivors-workplan.md` (delete-on-completion flips workflow to
...
`implementation-plans/loop-improvement-workplan.md`. Validation in sandbox:
...
| Gotcha | Pointer |
| :--- | :--- |
| ADR-11 PR-2 V2 rule scopes to `src/` only; V4/V11 to `tests/` only. `scripts/` and `verifiers/` are NOT authoring-guarded yet — small surface today, but a TCB-write regression in `scripts/check_protected_paths.py` would slip past `fa authoring-check`. | [`BACKLOG I-12`](./BACKLOG.md#i-12--authoring-rules-scope-coverage-gap-scripts-verifiers) |
| ADR-11 PR-2 V4 rule is bound to the literal `pytest.skip` / `pytest.mark.skip` attribute chain; `from pytest import skip; skip(...)` slips past it. Decorator form unaffected. **Scheduled to close in PR-13** (PR-10 follow-up, `pr13-v4-evasion-closure`). | [`BACKLOG I-13`](./BACKLOG.md#i-13--v4-import-alias-bypass-from-pytest-import-skip) |
| **PR-12 commit 9 of the PR-10 plan appends a NEW `I-14` (per-file decode caching), but `BACKLOG.md` already has a distinct `## I-14 — ADR-11 PR-3+ rule packs`.** Next agent MUST STOP at commit 9, classify as `plan-transcription error`, and renumber the new entries with the user before writing. | [`BACKLOG I-14`](./BACKLOG.md) |
...
1. ~~**Wire FIX-3: subagent_start/end via state.output_bus**~~ **DONE** — emit calls added to spawn_subagent.py at all 3 paths.

2. **Address NEW-5/NEW-6: stats gaps for circuit breaker + compaction starts** (~15 lines in `stats.py`).
   Parse `compaction_circuit_breaker` and `compaction_stage{2,3}_start` events. Detect incomplete compactions.

3. **Fix NEW-7: role column width in `fa stats --global-history`** (~2 lines).
   Increase `:<8s` to `:<20s` or truncate with ellipsis.

4. **LOGIC-10: abnormal_stop actionable guidance** (~10 lines in `coder_loop.py`).
   Add console hints for `abnormal_stop:length` and `abnormal_stop:content_filter`.

5. **LOGIC-16: read_all() O(n²)** — add `EventLog.count()` and `EventLog.tail(n)` methods.

6. **P6 (deferred): LOGIC-17, LOGIC-18, LOGIC-19.** Feature flag cascade, remove redundant `telemetry` event kind, remove dead `CompactionManager`.

7. **LOGIC-2: Workflow flow_state.json/eval_report.json not in event_log.**

8. **NEW-8: Move `import sqlite3` to top of state.py.** Style-only.

9. **NEW-9: Set output_bus in SessionState constructor** or document the None window.

10. **Skill universality pass:** The tests-writing skill was rewritten with FA-specific concepts (EventType, EventBus, OutputEvent, drive_session). The principles are universal but the framing is not. Needs an abstraction pass to separate universal rules from FA-specific examples. See analysis in `knowledge/research/skill-gaps-analysis-tests-writing-2026-07-19.md`.
