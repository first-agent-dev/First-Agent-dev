...
def setup():
    ...
    (tmp/"PLAN.md").write_text(plan); (tmp/"models.yaml").write_text("providers: {}\n")
    return tmp
...
def stage(a: argparse.Namespace, **kw: Any) -> int:
    if a.role=="coder": coder_calls["n"]+=1
    s=kw.get("outcome_sink")
    if a.role=="eval" and s is not None:
        s.append(SessionOutcome(exit_code=0,stop_reason="stopped_by_llm",turns=1,
          final_text="### Verdict\nPASS\n### Step results\n- S1: PASS - looks good to me\n"))
    return 0
rc,state=run_workflow(roles=["coder","eval"],task="implement the plan",per_role_task={},
  mode="adaptive",max_repairs=2,max_replans=0,run_id="q17demo",config=tmp/"models.yaml",
  workspace=tmp,max_turns=1,output_mode="quiet",run_stage_fn=stage,plan_path=tmp/"PLAN.md")
