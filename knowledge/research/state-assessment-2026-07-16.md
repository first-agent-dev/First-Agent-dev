...
| 3 | C1 tests for every product surface | ⚠️ PARTIAL | Missing: `fs_list_tasks` C1, subagent termination lifecycle C1, drive_session C1 for session DB + observability |
...
| **2** | **Task 3: `test_list_tasks_wiring.py`** | 45 min | Session wiring fixture (done) | `fs_list_tasks` registered but no C1 proves it works via live session; closes a Task 3 gap |
