---
title: "gortex + Aperant — inspiration deep-dive for First-Agent"
source:
  - "https://github.com/zzet/gortex"
  - "https://github.com/AndyMik90/Aperant"
compiled: "2026-05-13"
goal_lens: "Извлечь из gortex (code-intelligence + agent config audit) и Aperant (role-routing prompts + filesystem learning) паттерны для FA Mechanical Wiki + role-routing."
chain_of_custody: |
  Все упоминания файлов и LOC numbers — из соответствующих main branch
  репозиториев на дату 2026-05-13. Gortex: Go code-intelligence daemon +
  MCP server, 256 языков, 62 MCP tools, integrates 15 AI agents.
  Aperant: Electron app с planner/coder/qa role pipeline + GitHub PR
  automation.
---
...
### R-1 — Bloat/staleness audit for `knowledge/` + AGENTS.md (gortex `internal/audit/`)

- **What:** Lift gortex's 0-100 bloat scoring (lines, long lines, duplicate bullets, nesting depth, code blocks; soft cap 600, hard cap 1500) + stale-reference detection (extract backticked tokens, graph-validate symbols, stat-check paths) into a `fa audit` or `fa hygiene` CLI command. Outputs `StaleRef[] / DeadPath[] / BloatMetrics / Suggestions[]`.
- **Project-axis fit (stable across notes):**
  - (A) reduces session-start noise: YES (audit forces minimalism)
  - (B) helps LLM find context when needed: YES (dead-ref detection keeps refs valid)
- **Goal-lens fit (per session, dynamic):**
  - (C) advances chosen goal_lens: YES (Mechanical Wiki hygiene KPI made concrete)
- **Cost:** medium (3-5h skeleton — port heuristics + integrate with `knowledge/llms.txt` index)
- **Verdict:** TAKE
- **Alternative-if-rejected:** continue manual review; rely on rule #3 file-length caps in AGENTS.md PR Checklist
- **Concrete first step (if TAKE):** Read gortex `internal/audit/bloat.go` (158 LOC), `audit.go` (201 LOC), `tokens.go` (156 LOC) — port heuristic to Python in `src/fa/audit/`

### R-2 — Adapter pattern for per-agent config emission (gortex `internal/agents/`)

- **What:** Central spec → 15 adapters (CLAUDE.md, .cursorrules, AGENTS.md, .gemini, .windsurf, …). Marker-fenced regenerable blocks so generated + human content coexist. `Detect → Plan → Apply` contract for each integration.
- **Project-axis fit (stable across notes):**
  - (A) reduces session-start noise: NO (multi-agent emission not relevant for solo FA)
  - (B) helps LLM find context when needed: NO
- **Goal-lens fit (per session, dynamic):**
  - (C) advances chosen goal_lens: PARTIAL (marker-fenced regen blocks could apply to `knowledge/llms.txt` auto-regen post-Phase S)
- **Cost:** medium (3-4h marker-fence design)
- **Verdict:** DEFER (relevant only post-Phase S when llms.txt regenerator lands)
- **Alternative-if-rejected:** manual `llms.txt` updates per AGENTS.md PR Checklist rule #7
- **Concrete first step (if TAKE):** Add marker-fence convention to `knowledge/llms.txt` for the auto-regenerable section

### R-3 — Role-routing prompts library (Aperant `apps/desktop/prompts/`)

- **What:** Pre-built library: `planner / coder / qa_reviewer / qa_fixer / spec_*` + ~20 GitHub-workflow prompts (`pr_orchestrator`, `issue_analyzer`, `pr_security_agent`, `finding_validator`, `bot_detector`, etc.). High-density example of role-routed PR review.
- **Project-axis fit (stable across notes):**
  - (A) reduces session-start noise: NO (prompts are loaded per-task)
  - (B) helps LLM find context when needed: YES (role-routing means right prompt for right task)
- **Goal-lens fit (per session, dynamic):**
  - (C) advances chosen goal_lens: YES (role-routing prompts are core knowledge layer)
- **Cost:** medium (2-3h to extract + adapt 3-5 key prompts; full library too large)
- **Verdict:** TAKE (selective — adopt `pr_orchestrator` + `planner` + `qa_reviewer` shape)
- **Alternative-if-rejected:** continue using ad-hoc prompts in `knowledge/prompts/`
- **Concrete first step (if TAKE):** Read Aperant `apps/desktop/prompts/github/pr_orchestrator.md` + `pr_followup_orchestrator.md`; adapt to FA `knowledge/prompts/` structure

### R-4 — Filesystem-canonical learning loop (Aperant `record_discovery` / `record_gotcha`)

- **What:** Two minimal tools that append to `memory/codebase_map.json` and `memory/gotchas.md`. Almost exactly FA's Mechanical Wiki idiom — agent learns by writing to filesystem-canon Markdown/JSON.
- **Project-axis fit (stable across notes):**
  - (A) reduces session-start noise: YES (gotchas pre-loaded at session start)
  - (B) helps LLM find context when needed: YES (codebase_map.json = pointer index)
- **Goal-lens fit (per session, dynamic):**
  - (C) advances chosen goal_lens: YES (filesystem-canon learning is FA's stated design)
- **Cost:** cheap (1-2h — port two TypeScript tools to Python)
- **Verdict:** TAKE
- **Alternative-if-rejected:** continue using manual HANDOFF.md edits
- **Concrete first step (if TAKE):** Read Aperant `apps/desktop/src/main/ai/tools/auto-claude/record-{gotcha,discovery}.ts`; create FA equivalents

### R-5 — Phase-protocol orchestration (Aperant `build-orchestrator.ts` / `spec-orchestrator.ts`)

- **What:** Concrete phase-protocol implementation with validated transitions (planning → coding → qa_review → qa_fixing). Blueprint for FA's Phase R/S/M.
- **Project-axis fit (stable across notes):**
  - (A) reduces session-start noise: NO (runtime orchestration)
  - (B) helps LLM find context when needed: NO
- **Goal-lens fit (per session, dynamic):**
  - (C) advances chosen goal_lens: PARTIAL (Phase M runner inspiration, defers to future ADR)
- **Cost:** medium (3-4h to study + extract pattern)
- **Verdict:** DEFER (relevant when Phase M runner ADR lands)
- **Alternative-if-rejected:** design Phase M without phase-protocol reference
- **Concrete first step (if TAKE):** Read `build-orchestrator.ts` + `spec-orchestrator.ts`; document phase-transition validation pattern

### Summary

| R-N | Verdict | Project-fit (A / B) | Goal-fit (C) | Cost | Alternative-if-rejected | User decision needed? |
|-----|---------|---------------------|--------------|------|--------------------------|------------------------|
| R-1 | TAKE | YES / YES | YES | medium | manual review + rule #3 caps | Yes (priority vs B-NEW-5) |
| R-2 | DEFER | NO / NO | PARTIAL | medium | manual llms.txt updates | Yes (post-Phase S timing) |
| R-3 | TAKE | NO / YES | YES | medium | ad-hoc prompts | Yes (which 3-5 prompts to adopt) |
| R-4 | TAKE | YES / YES | YES | cheap | manual HANDOFF.md | No |
| R-5 | DEFER | NO / NO | PARTIAL | medium | design w/o reference | Yes (Phase M timing) |
