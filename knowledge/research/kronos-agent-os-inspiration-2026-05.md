---
title: "Kronos Agent OS (KAOS) — inspiration deep-dive for First-Agent"
source:
  - "https://github.com/spyrae/kronos-agent-os"
compiled: "2026-05-13"
goal_lens: "Извлечь из spyrae/kronos-agent-os (Python agent OS, ~9-7K LOC, SQLite-canonical, capability-gate-first) паттерны для FA Mechanical Wiki / inner-loop / B-NEW-* candidates."
chain_of_custody: |
  Все упоминания файлов и LOC numbers — из main-branch spyrae/kronos-agent-os
  на дату 2026-05-13 (shallow clone --depth=50). KAOS = Python agent OS,
  multi-agent (6 default), SQLite-canonical (SafeDB substrate), 4-layer
  memory stack (session.db + memory_fts.db + knowledge_graph.db + Qdrant),
  filesystem-canonical skills, capability-gated tools (5 env flags +
  Docker sandbox). Source: deep-dive note по 4 осям (Memory + Skills +
  Swarm + Capability gates), prepared 2026-05-13 в этой Devin-session.
---
...
> Six R-N candidates extracted from the «highest-leverage if only port 5
> things» list (§Highest-leverage) + the «Adjacent patterns worth
> stealing» appendix. Items that are duplicates of already-merged FA
> patterns (e.g. ADR-3/4 SQLite-FTS5 stack) are NOT promoted to R-N.

### R-1 — `SafeDB` lock-serialized SQLite substrate (`kronos/db.py`, 207 LOC)

- **What:** Single connection per database, `threading.Lock` around every
  read/write, WAL + `busy_timeout=30000` + `wal_autocheckpoint=100`,
  `BEGIN IMMEDIATE` for writes, auto-reconnect on `OperationalError`.
  KAOS's whole 4-layer memory stack sits on top of this.
- **Project-axis fit (stable across notes):**
  - (A) reduces session-start noise: NO (runtime substrate)
  - (B) helps LLM find context when needed: NO
- **Goal-lens fit (per session, dynamic):**
  - (C) advances chosen goal_lens: NO (FA's [ADR-4](../adr/ADR-4-storage-backend.md)
    chose external-content FTS5 single-writer; KAOS's multi-writer
    locking is over-engineered for v0.1)
- **Cost:** medium (~3-5h port + tests)
- **Verdict:** SKIP (no v0.1 multi-writer scenario; ADR-4 stdlib `sqlite3`
  is sufficient. Re-evaluate if v0.2 multi-process agents land.)
- **Alternative-if-rejected:** continue with ADR-4 stdlib `sqlite3` +
  per-tool `with sqlite3.connect(path)` idiom.
- **Concrete first step (if TAKE later):** Read `kronos/db.py` (207 LOC)
  in full; design FA equivalent at `src/fa/storage/safedb.py` only when
  the second writer (e.g. background sleep-pipeline daemon) needs to
  coexist with the foreground chunker.

### R-2 — Memory enrichment v0.2: Ebbinghaus FTS tiering + audit-JSONL redaction (`kronos/memory/fts.py` 306 LOC + `kronos/audit.py` ~250 LOC + `kronos/security/pii.py` 91 LOC)

- **What:** Two combined memory-layer adds: (1) FTS5 with explicit
  `relevance` + `tier` (active/warm/cold/archive) + `last_accessed`
  columns so the «Wiki gets too big» problem is solved by tier-shifting
  not deletion; (2) JSONL tool-call trail (`events.jsonl` analog) with
  recursive PII masking before write — UUIDs/hashes/tokens preserved,
  API keys + emails + secrets masked.
- **Project-axis fit (stable across notes):**
  - (A) reduces session-start noise: PARTIAL (tiering keeps cold facts
    out of context budget; PII masking is post-write only)
  - (B) helps LLM find context when needed: YES (decay surfaces fresh
    facts; redacted audit log is safe to bootstrap from)
- **Goal-lens fit (per session, dynamic):**
  - (C) advances chosen goal_lens: PARTIAL (alignsс ADR-3 v0.2 volatile-store
    hooks; ADR-7 §7 `events.jsonl` already exists without redaction)
- **Cost:** medium-high (5-8h: tier columns + decay schedule + PII
  recursive walker + audit-write hook)
- **Verdict:** DEFER (relevant when ADR-3 v0.2 volatile-store hooks
  land; redaction part may earlier surface if audit log starts being
  shared cross-session)
- **Alternative-if-rejected:** continue ADR-3 mechanical wiki without
  tiering; ADR-7 §7 raw `events.jsonl` без redaction.
- **Concrete first step (if TAKE later):** Read `kronos/memory/fts.py`
  (306 LOC, especially Ebbinghaus formula at lines 122-148) + `kronos/security/pii.py`
  (91 LOC, especially `mask_pii_object()` recursive walker).

### R-3 — Loop-detector circuit-breaker (`kronos/security/loop_detector.py` 99 LOC)

- **What:** 3-detector × 3-level escalation (WARN → CRITICAL →
  CIRCUIT_BREAKER) catching: (1) identical tool-call repeats; (2)
  near-identical action signatures within N turns; (3) thrash on the
  same file/path. On CIRCUIT_BREAKER → writes a NOTE to the trace
  («stop trying X»), forces role escalation or task abort.
- **Project-axis fit (stable across notes):**
  - (A) reduces session-start noise: NO (runtime)
  - (B) helps LLM find context when needed: YES (escalation NOTE
    becomes context for the next turn; «stop trying X» is exactly the
    kind of hard-won signal FA's HANDOFF.md captures)
- **Goal-lens fit (per session, dynamic):**
  - (C) advances chosen goal_lens: YES (Phase M inner-loop needs a
    «Coder fails loudly» mechanism that's more granular than ADR-2's
    «retry-once-then-stop»; loop-detector is the right shape)
- **Cost:** cheap (~99 LOC, no new deps, copy-pastable)
- **Verdict:** TAKE (Phase M, when inner-loop dispatcher lands)
- **Alternative-if-rejected:** rely on ADR-2 §Consequences «Coder
...
### R-4 — Filesystem-canonical skill store + safe community import (`kronos/skills/store.py` 339 + `tools.py` ~150 + `hub.py` 190 LOC)

- **What:** SKILL.md files with YAML frontmatter (`name`, `description`,
  `triggers`, `status: draft|active`, `review_required`); 3-section
  template (`## Trigger / ## Protocol / ## Output`); shared-overlay
  layering (project / user / global); `import_skill("github:user/repo/path")`
  with safe-slug validation, name-collision guard, mandatory
  `review_required: true` on every external import; `load_skill()`
  reveals draft skills with «say 'approve skill X' if useful» prefix
  (inverts the feature-flag model: visible but gated on user assent).
- **Project-axis fit (stable across notes):**
  - (A) reduces session-start noise: YES (skills are lazy-loaded by
    trigger; not all in context)
  - (B) helps LLM find context when needed: YES (Mechanical Wiki + 1
    trigger lookup → right skill loaded)
- **Goal-lens fit (per session, dynamic):**
  - (C) advances chosen goal_lens: YES (this is the most directly
    FA-compatible KAOS subsystem; matches
    [BACKLOG I-9](../BACKLOG.md#i-9--convert-knowledgepromptsrepo-audit-playbookmd-into-a-loadable-skill)
    «convert repo-audit-playbook into a loadable SKILL» perfectly)
- **Cost:** medium (~600 LOC of source to port; FA equivalent ~300-400
  LOC because no draft-status workflow yet)
- **Verdict:** TAKE (unblocks I-9; reference impl exists; design
  decisions pre-validated)
- **Alternative-if-rejected:** continue using flat
  `knowledge/prompts/*.md` files without status / triggers / draft
  workflow.
- **Concrete first step (if TAKE):** Read KAOS
  `kronos/skills/store.py` (339 LOC) + `kronos/skills/tools.py`
  (~150 LOC, especially `load_skill` reveal-with-prompt at lines 46-51).
  Convergence point с DPC B-NEW-7 (skill-store ADR) и Aperant R-4
  (filesystem-canonical learning loop) → 3-project convergence
  validates this is correct shape.

### R-5 — Identity-preserving compaction prompt (`kronos/memory/compaction.py:22-43`)

- **What:** A 5-line system-prompt fragment for any session-handoff
  summary task:
  > CRITICAL — preserve verbatim (do NOT paraphrase, do NOT omit):
  > UUIDs, hashes, tokens, IDs · URLs, hostnames, IPs, file paths ·
  > batch progress · decision status + reason · TODOs · names, dates,
  > sums · API keys masked

  Plus a 5-block structure mandate (Context / Decisions / Progress /
  Pending / Data) so the summary is shape-pinned.
- **Project-axis fit (stable across notes):**
  - (A) reduces session-start noise: YES (better handoff = fewer
    re-asks at session start; less re-derivation cost)
  - (B) helps LLM find context when needed: YES (UUID-style identifiers
    preserved verbatim → the next session can grep them)
- **Goal-lens fit (per session, dynamic):**
  - (C) advances chosen goal_lens: YES (HANDOFF.md quality is the
    closest FA-equivalent to compaction; this prompt is a direct upgrade)
- **Cost:** cheap (~5-line prompt fragment + 1-paragraph addition to
  HANDOFF.md «When to update» section; total ~15 LOC)
- **Verdict:** TAKE
- **Alternative-if-rejected:** continue with current HANDOFF.md
  freeform structure.
- **Concrete first step (if TAKE):** Verbatim copy the prompt fragment
  into `knowledge/prompts/handoff-summarizer.md` (new file); update
  `HANDOFF.md` §When to update this file» with cross-reference.

### R-6 — Cron-scheduled self-improvement + conversational approval (`kronos/cron/self_improve.py` 146 + `skill_create.py` 277 + `skill_improve.py` 177 + `sleep_compute.py` 237 LOC, plus `approve_skill(name)` chat-tool)

- **What:** Four cron jobs at staggered cadences (daily / weekly /
  nightly / nightly) that observe audit log → propose ONE concrete
  improvement, draft new skill, refine existing skills, consolidate
  memory. Pair with conversational approval: agent says «I drafted
  skill X — say 'approve skill X' to activate». No admin UI, no JSON
  payloads, no out-of-band approval flow.
- **Project-axis fit (stable across notes):**
  - (A) reduces session-start noise: PARTIAL (sleep_compute reduces
    `notes/` size by archiving stale facts → less noise at next start)
  - (B) helps LLM find context when needed: PARTIAL (skill_improve
    refines skills based on actual usage patterns)
- **Goal-lens fit (per session, dynamic):**
  - (C) advances chosen goal_lens: PARTIAL on `sleep_compute` only;
    NO on `self_improve` / `skill_create` / `skill_improve` —
    autonomous self-modification без external fitness signal
- **Cost:** high (~840 LOC for 4 cron files + tool registration +
  approval flow)
- **Verdict:** SKIP per
  [BACKLOG I-7 / I-8 Prior-art enforcement (DPC ADR-015, 2026-05-13)](../BACKLOG.md#i-7--bootstrap-cost-as-auto-collected-kpi-uc5-blocked):
  before any FA self-modification capability lands, eval-harness MUST
  be operational with external fitness signal. KAOS's cron suite +
  conversational approval is exactly the «elaborate-emptiness» shape
  DPC ADR-015 warned about (worker exists, fitness function absent →
  noise filtered by rules → 0 measurable improvement in 20+ sessions
  / ~40 proposals). The `approve_skill()` conversational approval is
  the «human is the fitness function» retrofit DPC also tried and
  removed.
- **Alternative-if-rejected:** keep skill creation manual (BACKLOG
  I-9 path); sleep-pipeline equivalent stays on DPC B-NEW-7 path
  (user-triggered, not cron) per
  [dpc-messenger-inspiration-2026-05.md §0 R-7](./dpc-messenger-inspiration-2026-05.md).
- **Concrete first step (if TAKE LATER, after I-7 + I-8 unblock):**
  Read `kronos/cron/sleep_compute.py` (237 LOC) first — это
  «memory consolidation», the lowest-risk of the four crons; map to
  DPC B-NEW-7 user-triggered shape, NOT KAOS cron shape.

### Summary table

| R-N | Verdict | Project-fit (A/B) | Goal-fit (C) | Cost | Alternative-if-rejected | User decision needed? |
|-----|---------|-------------------|--------------|------|------------------------|----------------------|
| R-1 | SKIP | NO / NO | NO | medium | continue stdlib `sqlite3` per ADR-4 | No (re-eval if v0.2 multi-writer) |
| R-2 | DEFER | PARTIAL / YES | PARTIAL | medium-high | continue ADR-3 wiki без tiering | No (post-Phase S) |
| R-3 | TAKE | NO / YES | YES | cheap | rely on ADR-2 «fail loudly» | Yes (priority vs B-NEW-5) |
| R-4 | TAKE | YES / YES | YES | medium | flat `knowledge/prompts/` без status | Yes (timing vs I-9) |
| R-5 | TAKE | YES / YES | YES | cheap | freeform HANDOFF.md | No (cheap; can land now) |
| R-6 | SKIP | PARTIAL / PARTIAL | PARTIAL | high | manual skill creation (I-9 path) | No (blocked by I-7/I-8 + DPC ADR-015) |
