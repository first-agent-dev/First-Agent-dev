...
└── systemd user service: fa.service        ← автозапуск при загрузке
    └── docker compose                       ← оркестратор (ДВА контейнера)
        ...
        └── контейнер: fa-egress-proxy       ← граница LLM-ключей (ADR-12 Option C)
            ...
            ├── /etc/fa/models.yaml → маршрутизация из /srv/first-agent/proxy (ro;
            │                          агент НЕ имеет к этому каталогу доступа на запись)
...
>
> ⚠️ **Источник истины для маршрутизации — `/srv/first-agent/state/models.yaml`**
