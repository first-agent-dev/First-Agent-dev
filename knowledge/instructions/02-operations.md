...
>
> #### 🧠 Про prompt-кэш: не забудь (коротко, чтобы не переоткрывать)
>
> **Правило одной фразы:** *prompt-кэш — это **возможность конкретного
> провайдера**, а не универсальная фича. Добавляя провайдера, ты обязан решить,
> умеет ли он кэшировать FA-овские параметры — и решить это по **измеренному**
> поведению, а не по догадке.*
>
> **Где живёт «ручка»:** в коде — `src/fa/providers/registry.py`, у каждого
> провайдера в `MessageRules` поле `supports_prompt_cache` (по умолчанию `true`).
>
...
> - **Не уверен** → гонишь живьём `fa conformance --provider <имя>` — он шлёт
>   **реальный** запрос и покажет 400, если провайдер их отвергает.
>
> **Железное правило:** это **флаг возможности, никогда не `if` по имени
> провайдера** (принципы D2/CT6). Пример: NVIDIA build не принимает эти
> параметры и отвечает 400 «Unsupported parameter(s)» — поэтому у него флаг
> выключен; OpenRouter/Together принимают и оставляют кэш включённым.
>
> ```bash
...
legacy-копии (`state/models.yaml` / `proxy/models.yaml`), нет ли лишней
директории `state/scripts/`, и расходится ли задеплоенный routing-файл с
checked-in шаблоном `knowledge/templates/models.yaml.example`. Скрипт ничего не
