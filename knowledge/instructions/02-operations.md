...
1. **Файл вне песочницы + запрет на чтение.** Ключи монтируются read-only в
   `/run/secrets/fa.env`, вне `/workspace`. `fs.read_file` ограничен
   path-containment (выход за `/workspace` запрещён). Для `fs.run_bash`
   read-команды (`cat`/`grep`/`dd`/`xxd`/…) **по умолчанию обходили**
   path-containment — поэтому добавлен fail-closed запрет: bash-gate
   отклоняет любую команду, читающую путь в `/run/secrets`,
   `/srv/first-agent/secrets` или `~/.fa/.env` (см. `src/fa/sandbox/secret_paths.py`).
   Это «растяжка» + защита deploy-ключа; для LLM-ключей основной барьер —
   egress-proxy (ключи вообще отсутствуют в контейнере агента, см. ниже).
