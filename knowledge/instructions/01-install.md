...
- **`first-agent`** — сам агент (`fs_run_bash` и т.д.). LLM-ключей у него нет.
...
- **Агент не может прочитать ключи.** Три барьера: файл вне `/workspace`
  (песочница `fs_read_file`/`bash` не выходит за её пределы); ключи не в
  `os.environ` (значит `printenv`/`/proc/self/environ`/`os.environ` пусты);
  `fs_run_bash` запускается с очищенным allowlist-окружением.
