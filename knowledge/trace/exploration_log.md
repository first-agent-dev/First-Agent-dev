...
## Q-20 — How to make bash stateful without breaking token efficiency and TCB? (2026-07-11)

- **Trigger:** Task "прочитай репозиторий и расскажи что нашел" → 124 steps, context accumulated each step, loop blocked by timeout. Root cause: stateless `subprocess.run(cwd=root, shell=True)` in `src/fa/inner_loop/tools/run_bash.py`, `cd` doesn't persist, no warning, no structured exploration.
- **Closed by:** [ADR-14](../adr/ADR-14-stateful-bash-eventstream-runtime.md) + implementation plan v3 reduced `knowledge/research/adr-13-14-implementation-plan-2026-07-11-v3-reduced.md`.
- **Chosen:** **EventStream Runtime FastAPI + PtyPool libtmux direct (Option C)** — shared `libtmux.Server` injected, LRU eviction + fail-fast `PoolExhaustedError` never reuse main, maxSize=2 (main+1 sub) for v0.1, sentinel `|||FA_READY|||`, ANSI strip regex, exit code parsing `__FA_EXIT__:$? __FA_END__`, fallback to pexpect with WARNING, DI via SessionState, no global singleton, thread-safe, defensive worktree checks Tier 1, prompt caching split cacheable=[BASE, AGENTS.md map, tool defs per role] + non-cacheable task+memory, cache-key = role_id + hash(names+schemas) + hash(agents_map) + hash(skills) (stable, no date), tool output offload 8000 chars + 500 preview per ArXiv 2603.05344 v2 staged compaction table, tool batching parallel read-only via ThreadPool max 5 with Lock sequential log write.
- **Rejected:**
  - **(a) Fake Stateful (cwd/env dict).** Reason: solves 80% cd waste but not `source .venv`, `npm run dev &`, interactive prompts, ANSI. Lesson: revisit if eval-harness shows cd persistence alone cuts 124→40 steps — then fake stateful sufficient for pair over autonomy.
...
- **Trigger:** After stateful PTY, need Cursor 3.2 /multitask: main breaks task into chunks, async subagents each isolated git worktree, restricted tools, structured JSON. Current workflow same toolset ~18 tools ~3000 tokens for all roles, no instant grep, `grep -ril` scans all files slow, no tool batching → 124 steps.
- **Closed by:** [ADR-15](../adr/ADR-15-multitask-subagents-worktree-isolation.md) + substrate formalization `knowledge/research/substrate-formalization-and-reduction.md`.
- **Chosen:** **Full Cursor-like but phased v0.1 SharedDir + 1 subagent limit + restricted toolset + instant grep FTS5 trigram + JSON envelope + skill globs foundation** — WorktreeManager ABC abstraction `create_subagent_workspace(task_id)->Path`, SharedDirWorktreeManager returns session_root 100% stable, IsolatedWorktreeManager `git worktree add .fa/worktrees/<id> -b agent/<id> origin/main` sanitized branch `re.sub(r'[^a-zA-Z0-9-_]', '-', task_id)[:50]`, defensive asserts after add path exists + is_dir + worktree list contains + branch already checked out fail-fast + CWD lock + cleanup assert not exists + prune. Profiles dynamic: researcher [glob,grep,read,instant_grep] 600 tokens vs full 3000 (-60%), verifier [bash] 200 tokens. Instant grep FTS5 trigram `tokenize='trigram'` DELETE then INSERT + mtime tracking + stale cleanup + fallback porter WARNING, returns paths <50ms substring search "auth"→"AuthMiddleware", token efficient, 0 deps sqlite3 stdlib, excludes .fa/, node_modules/. SubagentEnvelope full schema Goal, Verification, Risks, token_usage, duration_ms, next_action, fastjsonschema validator cached, artifact write .fa/subagents/<id>.json, filtered history task+relevant files from instant_grep not full parent 124 steps. Skill globs frontmatter `globs, alwaysApply: false` loader checks globs matches current_files or triggers verb matches, like Cursor Rules. Security arbiter foundation: SubagentRunner accepts separate proxy_token var for future per-subagent random token rewriting. Orchestration hybrid: planner writes spawn in Plan, coder executes as step. 1 subagent limit enforced via RuntimeLimits.max_subagent_spawns_per_session=3. Pair over Autonomy principle: checkpoint/undo/diff deterministic Python, not LLM.
- **Rejected:**
  - **(a) Keep same toolset for all roles, same worktree shared dir, no instant grep.** Reason: token bloat 3000 vs 600, no isolation, grep scans all files slow → 124 steps. Lesson: never — violates Pillar 3.
  - **(b) Full 8 parallel isolated worktrees + persistent per agent + merge lock + fleet (Cursor 3.2 full).** Reason: topology complexity symptom of missing formal substrate (Paper 2 §4.4). User philosophy: self-evolving 100k logs about nothing is want/FOMO, enough to silently think and do one task together as pair. Don't believe in smart autonomous systems now. Subagent idea solution to problem not yet formed by practice. Keep 1 subagent limit for v0.1 to eliminate scope creep, prove simple chain works before adding parallel tree. Lesson: re-opens when eval-harness 5 tasks proves simple chain insufficient and practice-formed task emerges — then Phase 4 Remote Runtime Extraction + parallel 2-3.
  - **(c) Only worktree isolation, no toolset restriction, no instant grep.** Reason: still token heavy, no fast search. Lesson: isolation alone insufficient.
- **Coupling:** Q-20 (stateful PTY), Q-19 (workspace isolation read-only mount + clone), Q-8 (HookRegistry), ADR-12 arbiter pattern per-subagent random token foundation, Paper 2 §4.4 topology complexity vs formality, §1.2.6 Substrate Formality Principle, §1.2.7 Pair over Autonomy Principle.
- **Source:** [ADR-15](../adr/ADR-15-multitask-subagents-worktree-isolation.md), [Substrate Formalization](../../research/substrate-formalization-and-reduction.md), Cursor 3.2 /multitask, Copilot CustomAgents tool restriction + fleet, OpenAI Sandbox as Tools JSON, Cursor instant grep N-gram 3 months prod, LangChain subagents, Claude Code worktree bugs #55708 #47548 #31546, final-review-gaps-high-roi-metaharnesses-july2026.md Gaps 4,6,8,10,11.

## Q-22 — When to introduce formal substrate principles and pair over autonomy? (2026-07-11)

- **Trigger:** Implementation plan v2 had 30+ components interacting per session: artefacts, traces, anti-patterns, research, adr, memory tiers, prompts, skills, guardrails, roles, tool schemas, blackboard, telemetry, FTS index, worktrees, PtyPool, subagent envelopes, task worklogs. Adding new component required updating 5-6 others — breaking point N^2. User observed self-evolving systems that produce 100k logs about nothing are want/FOMO, occupy neurodivergent individuals, enough to silently think and do one task together with agent as pair.
- **Closed by:** [project-overview.md §1.2.6, §1.2.7](../../../project-overview.md#126-substrate-formality-principle--formal-substrate-before-topology-complexity) + [ADR-16](../adr/DIGEST.md) row + `knowledge/research/substrate-formalization-and-reduction.md` + `philosophy-subagents-cheap-deterministic.md`.
- **Chosen:** **Add two new enforceable principles §1.2.6 Substrate Formality Principle and §1.2.7 Pair over Autonomy Principle** — Substrate Formality: Topology complexity is symptom of missing formal substrate. When need to add parallel agents, dynamic DAGs, workflow mutation, agent pool scaling, fleet, elaborate adaptive topologies to fix token efficiency, context window, coordination, first check if missing formal shared substrate would allow simple chain to work. Invariants I-6.1 every write declares read_set/write_set/assumptions/version_dependencies, I-6.2 blackboard append-only content-hashed queryable detect_conflict(), I-6.3 no silent overwrite → fail code conflict_detected, I-6.4 simple chain planner→coder→eval default, parallel only when substrate formal and task embarrassingly parallel non-overlapping write_sets. Pair over Autonomy: Agent should work as pair programming partner, not autonomous system. Main 180k context, subagent clean slate ~1k never inherits full parent history, task solvable with <600 tokens tool defs and <8000 output structured JSON, stateless scrubbed env isolated, no self-evolving harness without eval-harness proving simple chain insufficient and human approval for permission boundaries. Invariants I-7.1..I-7.5. Implementation plan v3 reduced: 5 phases total (0,0.5,1,2,3) — 4 main after quick-win, closer to pair philosophy "молча подумать и сделать одну задачу вместе". Removed Phase 4 Remote Runtime Extraction and parallel 2-3 tree until proven needed.
- **Rejected:**
  - **(a) Keep 6 phases including Remote Runtime Extraction FastAPI separate service + parallel 2-3 tree.** Reason: big teams start simple, extract to service only when parallel need proven. Interface BashExecutor allows switch to remote later without changing callers. Pair over autonomy prefers simple chain. Lesson: re-opens when Phase 3 shows instability or need 2-3 parallel proven — not now.
