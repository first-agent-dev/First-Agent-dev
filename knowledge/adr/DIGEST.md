...
## ADR-15 — Multitask Subagents Worktree Isolation, Toolset per Role, Instant Grep, JSON Envelope (proposed 2026-07-11)

**Decision.** Multitask subagents via WorktreeManager abstraction with defensive checks Tier 1 per external reviewer: SharedDirWorktreeManager v0.1 (1 subagent limit, shared dir, sequential single-shot, 100% stable) → IsolatedWorktreeManager future `git worktree add .fa/worktrees/<id> -b agent/<id> origin/main` separate dir, sanitized branch `re.sub(r'[^a-zA-Z0-9-_]', '-', task_id)[:50]`, defensive asserts path exists + is_dir + worktree list contains + branch already checked out fail-fast clear error + CWD lock + cleanup assert not exists + prune. Profiles dynamic toolset: researcher [glob,grep,read,instant_grep] 600 tokens vs full 3000 (-60%), verifier [bash] 200 tokens, code-reviewer [read,grep], implementer full, planner read-only. Toolset per role dynamic main chooses at spawn via PROFILES dict, cache-key per role solves internal contradiction. Instant grep FTS5 trigram `tokenize='trigram'` with DELETE then INSERT + mtime tracking + stale cleanup + fallback porter WARNING, returns paths <50ms substring search "auth"→"AuthMiddleware", token efficient, 0 external deps sqlite3 stdlib. SubagentEnvelope full schema Goal, Verification, Risks, token_usage, duration_ms, next_action, fastjsonschema validator cached, artifact write .fa/subagents/<id>.json, filtered history task+relevant files from instant_grep not full parent 124 steps, JSON envelope reduces context accumulation. Task worklog.md per task Goal, Evidence, Steps, Verification aggregated from JSONs, for PR → PR body. Skill globs frontmatter `globs, alwaysApply: false` loader checks globs matches current_files or triggers verb matches, like Cursor Rules. Security arbiter foundation: SubagentRunner accepts separate proxy_token var for future per-subagent random token rewriting, foundation for Gap 7 arbiter pattern. Orchestration hybrid: planner writes spawn in Plan, coder executes as step, compliance-by-construction. 1 subagent limit enforced via RuntimeLimits.max_subagent_spawns_per_session=3. **Rationale.** Cursor 3.2 /multitask main breaks into chunks async subagents each git worktree isolated, 8 parallel, Copilot CustomAgents tool restriction, OpenAI Sandbox as Tools custom_output_extractor JSON, Cursor instant grep N-gram 3 months prod. Topology complexity is symptom of missing formal substrate — fix substrate before adding parallel agents.

**Source:** [`ADR-15`](./ADR-15-multitask-subagents-worktree-isolation.md). [Formerly ADR-14 in draft.]

## ADR-16 — Substrate Formality and Pair over Autonomy Principles (proposed 2026-07-11)

**Decision.** Add two new enforceable principles §1.2.6 Substrate Formality Principle and §1.2.7 Pair over Autonomy Principle next to minimalism-first §1.2 and compliance-by-construction §1.2.5. Substrate Formality: Topology complexity is symptom of missing formal substrate. When you feel need to add parallel agents, dynamic DAGs, workflow mutation, agent pool scaling, fleet, elaborate adaptive topologies to fix token efficiency, context window, coordination, first check if missing formal shared substrate would allow simple chain to work. Invariants I-6.1 every write declares read_set/write_set/assumptions/version_dependencies, I-6.2 blackboard append-only content-hashed queryable detect_conflict(), I-6.3 no silent overwrite → fail code conflict_detected, I-6.4 simple chain planner→coder→eval default, parallel only when substrate formal and task embarrassingly parallel non-overlapping write_sets. Pair over Autonomy: Agent should work as pair programming partner, not autonomous system. Main 180k context, subagent clean slate ~1k never inherits full parent history, task solvable with <600 tokens tool defs and <8000 output structured JSON, stateless scrubbed env isolated, no self-evolving harness without eval-harness proving simple chain insufficient and human approval for permission boundaries. Invariants I-7.1..I-7.5. **Rationale.** User philosophy: self-evolving systems that produce 100k logs about nothing are want/FOMO, occupy neurodivergent minds, enough to silently think and do one task together as pair. Don't believe in smart autonomous systems now. Subagent idea solution to problem not yet formed by practice, except cheap deterministic puzzle piece when main 180k near limit. Paper 2 §4.4 topology complexity inversely correlates with harness-state formality: explicit formal substrates use simple topologies (L2MAC). Paper 2 §5.1.1 Harness as Distillation Surface: production harnesses becoming training data.

**Source:** [`project-overview.md` §1.2.6, §1.2.7](../project-overview.md#126-substrate-formality-principle--formal-substrate-before-topology-complexity)
