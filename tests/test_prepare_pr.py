...
def test_happy_path_fix_emits_all_clauses(draft_store: PrDraftStore, draft_path: Path) -> None:
    result = _invoke(
        draft_store,
        {
            ...
            "deterministic_mechanism": ("raise SystemExit(2) before chain construction; src/fa/cli.py:298"),
        },
    )
    assert result.error is None, result.error
    text = draft_path.read_text(encoding="utf-8")
...
def test_non_fix_with_degree_of_freedom_closed_refused(draft_store: PrDraftStore, draft_path: Path) -> None:
    result = _invoke(
        draft_store,
        {
            "intent": "IMPLEMENT",
...
def test_non_fix_with_deterministic_mechanism_refused(draft_store: PrDraftStore, draft_path: Path) -> None:
    result = _invoke(
        draft_store,
        {
            "intent": "IMPLEMENT",
