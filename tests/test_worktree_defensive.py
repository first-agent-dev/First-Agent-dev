...
def test_isolated_manager_branch_already_checked_out():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        ws1 = manager.create_subagent_workspace("task1", base_branch="main" if (repo / ".git" / "refs" / "heads" / "main").exists() else "master")
        assert ws1.exists()
        ...
        # But if we manually try to create same branch via different task_id that maps to same branch name? Our branch name is agent/<task_id> unique, so no collision
        ...
        is_checked, _ = manager._is_branch_checked_out_elsewhere(f"agent/collision-test")
