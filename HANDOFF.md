...
**As of:** 2026-06-08 — Secrets Hardening PR (branch `devin/secrets-hardening`): Container integration (bind-mount `~/.fa` in compose, `.env.fa.template`, `models.yaml.example`), runtime redaction (`SecretRedactor` + 7 regression tests wired into `EventLog`, `LearningObserver`, `AuditHook`), agent self-protection (`SecretGuard` hook), developer onboarding (`~/.fa/.env` optional loader), deployment docs updated (`SETUP_AIO.md` Phase 7b + security notes), repo hygiene (`.gitignore`, `.dockerignore`, `.gitleaks.toml` allowlist), README security section. **1002 tests, ruff clean.**
